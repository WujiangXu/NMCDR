...
def test(model,args,valLoader):
    model.eval()
    stats = AverageMeter('loss','loss_cls','loss_cl')
    ...
    return stats.loss, stats.loss_cls, stats.loss_cl, HIT_1_d1, NDCG_1_d1, HIT_5_d1, NDCG_5_d1, HIT_10_d1, NDCG_10_d1, MRR_d1, HIT_1_d2, NDCG_1_d2, HIT_5_d2, NDCG_5_d2, HIT_10_d2, NDCG_10_d2, MRR_d2
...
def train(model,trainLoader,args,valLoader):
    ...
    for epoch in range(args.epoch):
        ...
        for i,sample in enumerate(tqdm(trainLoader)):
            ...
            labels = torch.LongTensor(sample['label'].long()).cuda()
            labels = labels.float()
            # print(u_node,i_node,seq_d1,seq_d2,domain_id,neg_samples)
            # print(u_node.shape,i_node.shape,seq_d1.shape,seq_d2.shape,domain_id.shape,neg_samples.shape,labels.shape)
            # pos_logit_d1, neg_logit_d1, pos_logit_d2, neg_logit_d2 = model(u_node,i_node,seq_d1,seq_d2,domain_id,neg_samples)
            # predict_d1, predict_d2, u_feat_enhance_m1_d1, u_feat_enhance_m1_d2, u_feat_enhance_m2_d1,u_feat_enhance_m2_d2, u_feat_enhance_m3_d1,u_feat_enhance_m3_d2, u_feat_enhance_m4_d1, u_feat_enhance_m4_d2 = model(u_node,i_node,neg_samples,seq_d1,seq_d2,long_tail_mask_d1,long_tail_mask_d2)
            predict_d1, predict_d2, predict_m0_d1, predict_m0_d2, predict_m1_d1, predict_m1_d2, predict_m2_d1, predict_m2_d2, predict_m3_d1, predict_m3_d2 = model(u_node,i_node,neg_samples,seq_d1,seq_d2,long_tail_mask_d1,long_tail_mask_d2)
            # labels = torch.LongTensor(torch.zeros(predict_d1.shape[0]).long()).cuda()
            # if i==0:
            #     logger.info(f'u_feat_enhance_m1_d1:{u_feat_enhance_m1_d1}, u_feat_enhance_m1_d2:{u_feat_enhance_m1_d2}  \t')
            #     logger.info(f'logits :{torch.exp(F.normalize(torch.matmul(u_feat_enhance_m1_d1,u_feat_enhance_m1_d2.T),p=1,dim=1))/0.07}  \t')
            predict_d1 = predict_d1.squeeze()
            predict_d2 = predict_d2.squeeze()
            one_value = torch.LongTensor(torch.ones(domain_id.shape[0]).long()).cuda()
            mask_d1 = torch.LongTensor((one_value.cpu() - domain_id.cpu()).long()).cuda()
            ...
            # label_domain = torch.LongTensor([0,1,0,1]).cuda().float()
            # loss_cl = nn.BCELoss()(predict_domain.squeeze(),label_domain)
            # loss_cl = cal_loss_cl_all(u_feat_enhance_m1_d1, u_feat_enhance_m1_d2, u_feat_enhance_m2_d1,u_feat_enhance_m2_d2, u_feat_enhance_m3_d1,u_feat_enhance_m3_d2, u_feat_enhance_m4_d1, u_feat_enhance_m4_d2)# 
            
            # loss_cl =  cal_loss_cl_refine(u_feat_enhance_m2_d1,u_feat_enhance_m2_d2)#+cal_loss_cl_refine(u_feat_enhance_m3_d2,u_feat_enhance_m4_d2)#+cal_loss_cl_refine(u_feat_enhance_m2_d1,u_feat_enhance_m2_d2)+cal_loss_cl_refine(u_feat_enhance_m3_d1,u_feat_enhance_m3_d2)+cal_loss_cl_refine(u_feat_enhance_m4_d1,u_feat_enhance_m4_d2) #+ cal_loss_cl_refine(u_feat_enhance_m3_d2, u_feat_enhance_m4_d2)
            loss_cls_m3 = criterion_cls(predict_m3_d1,labels) * mask_d1.unsqueeze(1)  + criterion_cls(predict_m3_d2,labels) * mask_d2.unsqueeze(1) #* 2
            loss_cls_m3 = torch.mean(loss_cls_m3)
            ...
            # loss_cls_m0 = criterion_cls(predict_m0_d1,labels) * mask_d1.unsqueeze(1) + criterion_cls(predict_m0_d2,labels) * mask_d2.unsqueeze(1) #* 2
            # loss_cls_m0 = torch.mean(loss_cls_m0)
            # loss_cl =  cal_loss_cl_refine(u_feat_enhance_m1_d1,u_feat_enhance_m4_d1)+cal_loss_cl_refine(u_feat_enhance_m1_d2,u_feat_enhance_m4_d2)+cal_loss_cl_refine(u_feat_enhance_m4_d1,u_feat_enhance_m4_d2)
            loss = loss_cls + loss_cls_m1 + loss_cls_m2 + loss_cls_m3 #+ loss_cls_m0 #+ loss_cl * 0.05
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # auc_avg_domain = roc_auc_score(labels.detach().cpu().numpy(),predict.detach().cpu().numpy())
            stats.update(loss=loss.item(),loss_cls=loss_cls.item(),loss_cls_m3=loss_cls_m3.item(),loss_cls_m2=loss_cls_m2.item(),loss_cls_m1=loss_cls_m1.item())
            if i % 20 == 0:
                logger.info(f'train total loss:{stats.loss}, cls loss:{stats.loss_cls}, cls m1 loss:{stats.loss_cls_m1}, cls m2 loss:{stats.loss_cls_m2}, cls m3 loss:{stats.loss_cls_m3} \t')
            #print("epoch :{} train loss:{}, auc:{}".format(epoch,stats.loss,stats.auc)) 
        #val(epoch)
        val_loss, val_cls_loss, val_rec_loss, HIT_1_d1, NDCG_1_d1, HIT_5_d1, NDCG_5_d1, HIT_10_d1, NDCG_10_d1, MRR_d1, HIT_1_d2, NDCG_1_d2, HIT_5_d2, NDCG_5_d2, HIT_10_d2, NDCG_10_d2, MRR_d2 = test(model,args,valLoader)
        ...
        logger.info(f'Epoch: {epoch}/{args.epoch} \t'
                    ...
                    f'Val loss: {val_loss:.4f}, cls loss: {val_cls_loss:.4f}, cl loss: {val_rec_loss:.4f} \n'
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        datasetTrain = DualDomainSeqDataset(seq_len=args.seq_len,isTrain=True,neg_nums=args.neg_nums,long_length=args.long_length,pad_id=item_length+1,csv_path="music_movie_train50.csv")#+str(int(args.overlap_ratio*100))+".csv")
        trainLoader = data.DataLoader(datasetTrain, batch_size=args.bs, shuffle=True, num_workers=8,collate_fn=collate_fn_enhance)

        datasetVal = DualDomainSeqDataset(seq_len=args.seq_len,isTrain=False,neg_nums=args.neg_nums,long_length=args.long_length,pad_id=item_length+1,csv_path="music_movie_test.csv")
        ...
        model = NGMCDR(user_length=user_length, user_emb_dim=args.emb_dim, item_length=item_length, item_emb_dim=args.emb_dim, seq_len=args.seq_len, m1_layers=args.m1_layers, m2_layers=args.m2_layers, m3_layers=args.m3_layers, m4_layers=args.m4_layers, hid_dim=args.hid_dim).cuda()
